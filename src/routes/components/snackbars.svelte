...
<div class="py-2">
  <Button color="secondary" on:click={() => (showSnackbarTop = true)}>
    Show snackbar on top
  </Button>
</div>
<div class="py-2">
  <Button color="alert" on:click={() => (showSnackbarBottomLeft = true)}>
    Show snackbar on the bottom left
  </Button>
</div>

<p class="mt-10">
  Also Smelte comes with a simple notification queue implementation.
</p>

<TextField bind:value={message} label="Message text" />
<Button disabled={!message} on:click={notify}>Add Notification to queue</Button>

<Button disabled={!message} color="alert" on:click={alert}>
  Alert message
</Button>

<Button disabled={!message} color="error" on:click={error}>
  Error message
</Button>
